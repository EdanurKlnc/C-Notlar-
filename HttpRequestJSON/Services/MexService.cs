...
namespace HttpRequestJSON.Services
{
    public class MexService
    {
        ...
        public bool CheckApiStatus()
        {
            ...
            HttpResponseMessage response = client.GetAsync("/api/v3/time").Result; //istek atma

            if (response.StatusCode == HttpStatusCode.OK) //Bağlantı kurarken hata olup olamdığını kontrol ediyoruz
            {
